...
export abstract class AbstractExtensionService extends Disposable implements IExtensionService {
	...
	constructor(
		...
		this._register(
			this._extensionManagementService.onDidChangeProfile(({ added, removed }) => {
	...
	private async _updateExtensionsOnExtHosts(
		...
		const promises = this._extensionHostManagers.map(extHostManager =>
			this._updateExtensionsOnExtHost(extHostManager, versionId, toAdd, toRemove, removedRunningLocation)
		);
	...
	private async _updateExtensionsOnExtHost(
		...
		const myToRemove = filterExtensionIdentifiers(toRemove, removedRunningLocation, extRunningLocation =>
	...
	private _canAddExtension(extension: IExtensionDescription, extensionsBeingRemoved: IExtensionDescription[]): boolean {
		...
		if (existing) {
			const isBeingRemoved = extensionsBeingRemoved.some(extensionDescription =>
				ExtensionIdentifier.equals(extension.identifier, extensionDescription.identifier)
			);
			if (!isBeingRemoved) {
				return false;
			}
		...
		if (!shouldActivateReason && hasWorkspaceContains) {
			...
			const host: IWorkspaceContainsActivationHost = {
				...
				checkExists: (folders, includes, token) =>
					this._instantiationService.invokeFunction(accessor => checkGlobFileExists(accessor, folders, includes, token))
			};
		...
		for (const activationEvent of this._pendingRemoteActivationEvents) {
			const result = Promise.all(
				remoteExtensionHosts.map(extHostManager =>
					extHostManager.activateByEvent(activationEvent, ActivationKind.Normal)
				)
			).then(() => {});
			...
			if (allExtensions.length < toAdd.length + resolverExtensions.length) {
				const toRemove = resolverExtensions.filter(
					registered =>
						!allExtensions.some(
							e =>
								ExtensionIdentifier.equals(e.identifier, registered.identifier) &&
	...
	protected async _resolveAuthorityOnExtensionHosts(
		...
		const results = await Promise.all(
			extensionHosts.map(extHost => extHost.resolveAuthority(remoteAuthority, this._resolveAuthorityAttempt))
		);
			...
			veto(value, reason) {
				...
				if (typeof value === 'boolean') {
					...
					value
						...
						.catch(error => {
							vetoReasons.add(
								nls.localize('extensionStopVetoError', '{0} (Error: {1})', reason, toErrorMessage(error))
							);
						});
				}
			}
	...
	public readExtensionPointContributions<T extends IExtensionContributions[keyof IExtensionContributions]>(
				...
				if (desc.contributes && hasOwnProperty.call(desc.contributes, extPoint.name)) {
					result.push(
						new ExtensionPointContribution<T>(
							desc,
							desc.contributes[extPoint.name as keyof typeof desc.contributes] as T
						)
					);
				}
			}
	...
	private static _handleExtensionPoint<T extends IExtensionContributions[keyof IExtensionContributions]>(
	...
	private _onDidActivateExtension(
		...
		extensionStatus.setActivationTimes(
			new ActivationTimes(codeLoadingTime, activateCallTime, activateResolvedTime, activationReason)
		);
