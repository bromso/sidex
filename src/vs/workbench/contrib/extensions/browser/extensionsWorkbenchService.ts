	...
	private get resourceExtensionIconUrl(): string | undefined {
		if (this.resourceExtension?.manifest.icon) {
			return FileAccess.uriToBrowserUri(
				resources.joinPath(this.resourceExtension.location, this.resourceExtension.manifest.icon)
			).toString(true);
		}
		return undefined;
	}
...
class Extensions extends Disposable {
	...
	constructor(
		...
		if (this.isWorkspaceServer) {
			...
			this._register(
				this.workbenchExtensionManagementService.onDidUninstallExtension(e => {
					if (e.workspaceScoped) {
	...
	private async mapInstalledExtensionWithCompatibleGalleryExtension(
		...
		if (compatibleGalleryExtensionsToFetch.length) {
			const result = await this.galleryService.getExtensions(
				...
				{ targetPlatform, compatible: true, queryAllVersions: true, productVersion },
				CancellationToken.None
			);
		...
		if (localExtension.manifest.version !== gallery.version) {
			...
			const galleryWithLocalVersion: IGalleryExtension | undefined = (
				await this.galleryService.getExtensions(
					[{ ...localExtension.identifier, version: localExtension.manifest.version }],
					CancellationToken.None
				)
			)[0];
	...
	private onInstallExtension(event: InstallExtensionEvent): void {
		...
		if (source && !URI.isUri(source)) {
			const extension =
				this.installed.find(e => areSameExtensions(e.identifier, source.identifier)) ??
	...
	private async matchInstalledExtensionsWithGallery(extensions: Extension[]): Promise<void> {
		...
		const galleryExtensions = await this.galleryService.getExtensions(
			...
			{ compatible: true, targetPlatform: await this.server.extensionManagementService.getTargetPlatform() },
			CancellationToken.None
		);
		for (const extension of extensions) {
	...
	private onDidUninstallExtension({ identifier, error }: DidUninstallExtensionEvent): void {
		const uninstalled =
			this.uninstalling.find(e => areSameExtensions(e.identifier, identifier)) ||
	...
	private onEnablementChanged(platformExtensions: readonly IPlatformExtension[]) {
		const extensions = this.local.filter(e =>
			platformExtensions.some(p => areSameExtensions(e.identifier, p.identifier))
		);
		for (const extension of extensions) {
			if (extension.local) {
	...
	getExtensionState(extension: Extension): ExtensionState {
		...
		const local = this.installed.filter(
			e =>
				e === extension ||
				(e.gallery && extension.gallery && areSameExtensions(e.gallery.identifier, extension.gallery.identifier))
		)[0];
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
	...
	private async initialize(): Promise<void> {
		...
		this._register(
			this.storageService.onDidChangeValue(
				...
				this._store
			)(_e => this.onDidDismissedNotificationsValueChange())
		);
		this._register(
			this.storageService.onDidChangeValue(
				...
				this._store
			)(_e => this.onDidSelectedExtensionToAutoUpdateValueChange())
		);
		this._register(
			this.storageService.onDidChangeValue(
				StorageScope.APPLICATION,
	...
	private async onDidChangeRunningExtensions(
		...
		for (const desc of added) {
			const extension = this.installed.find(e =>
				areSameExtensions({ id: desc.identifier.value, uuid: desc.uuid }, e.identifier)
			);
			if (extension) {
	...
	async getResourceExtensions(locations: URI[], isWorkspaceScoped: boolean): Promise<IExtension[]> {
		...
		return resourceExtensions.map(
			resourceExtension =>
				this.getInstalledExtensionMatchingLocation(resourceExtension.location) ??
				this.instantiationService.createInstance(
					Extension,
					ext => this.getExtensionState(ext),
					ext => this.getRuntimeState(ext),
		...
		if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
			const restartRequiredExtensions = this.local.filter(
				e =>
					...
					(e.runtimeState.action === ExtensionRuntimeActionType.RestartExtensions ||
	...
	private getInstalledExtensionMatchingLocation(location: URI): IExtension | null {
		return (
			this.local.find(e => e.local && this.uriIdentityService.extUri.isEqualOrParent(location, e.local?.location)) ??
			null
		);
	}
		...
		for (const extension of extensionsToCheck) {
			...
			if (isEnabled) {
				const runningExtension = this.extensionService.extensions.find(e =>
					areSameExtensions({ id: e.identifier.value, uuid: e.uuid }, extension.identifier)
				);
				if (runningExtension) {
					...
					if (isSameExtensionRunning) {
						...
						if (this.extensionsServers.length > 1) {
							...
							if (extensionInOtherServer) {
								...
								if (
									runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer &&
	...
	private getExtensionState(extension: Extension): ExtensionState {
		if (
			this.installing.some(
				i =>
					areSameExtensions(i.identifier, extension.identifier) && (!extension.server || i.server === extension.server)
			)
		) {
			return ExtensionState.Installing;
		}
	...
	async checkForUpdates(reason?: string, onlyBuiltin?: boolean): Promise<void> {
		...
		for (const installed of this.local) {
			...
			if (
				...
				(installed.type === ExtensionType.System || !installed.local?.identifier.uuid)
			) {
		...
		if (infos.length) {
			...
			const galleryExtensions = await this.galleryService.getExtensions(
				infos,
				{ targetPlatform, compatible: true, productVersion: this.getProductVersion() },
				CancellationToken.None
			);
		...
		this.progressService.withProgress({ location: ProgressLocation.Notification }, async progress => {
			try {
				...
				this.notificationService.error(
					nls.localize('download.failed', 'Error while downloading the VSIX: {0}', getErrorMessage(error))
				);
			}
		});
	}
	...
	private eventuallyAutoUpdateExtensions(): void {
		this.autoUpdateDelayer.trigger(() => this.autoUpdateExtensions()).then(undefined, _err => null);
	}
	...
	private async autoUpdateBuiltinExtensions(): Promise<void> {
		...
		await Promises.settled(
			toUpdate.map(e => this.install(e, e.local?.preRelease ? { installPreReleaseVersion: true } : undefined))
		);
	}
	...
	private async autoUpdateExtensions(): Promise<void> {
		...
		await Promises.settled(
			toUpdate.map(e =>
				this.install(e, e.local?.preRelease ? { installPreReleaseVersion: true, productVersion } : { productVersion })
			)
		);
	}
	...
	async canInstall(extension: IExtension): Promise<true | IMarkdownString> {
		if (!(extension instanceof Extension)) {
			return new MarkdownString().appendText(
				nls.localize('not an extension', 'The provided object is not an extension.')
			);
		}

		if (extension.isMalicious) {
			return new MarkdownString().appendText(
				nls.localize('malicious', 'This extension is reported to be problematic.')
			);
		}

		if (extension.deprecationInfo?.disallowInstall) {
			return new MarkdownString().appendText(
				nls.localize('disallowed', 'This extension is disallowed to be installed.')
			);
		}
	...
	async install(
		...
		if (arg instanceof URI) {
			...
			if (installableInfo) {
				const targetPlatform = extension?.server
					...
					: undefined;
				gallery = (
					await this.galleryService.getExtensions([installableInfo], { targetPlatform }, CancellationToken.None)
				).at(0);
			}
				...
				if (!installable) {
					if (!gallery) {
						...
						const reportIssueUri = manifest
							? getExtensionGalleryManifestResourceUri(manifest, ExtensionGalleryResourceType.ContactSupportUri)
							: undefined;
		...
		if (installable) {
			if (installOptions.justification) {
				...
				buttons.push({
					label:
						isString(installOptions.justification) || !installOptions.justification.action
							? nls.localize({ key: 'installButtonLabel', comment: ['&& denotes a mnemonic'] }, '&&Install Extension')
							: nls.localize(
									{ key: 'installButtonLabelWithAction', comment: ['&& denotes a mnemonic'] },
			...
			if (installable instanceof URI) {
				...
				if (extension.resourceExtension) {
					...
					extension = await this.doInstall(
						extension,
						() => this.installFromGallery(extension!, installable as IGalleryExtension, installOptions, servers),
						progressLocation
					);
				}
			}
				...
				if (installOptions.justification) {
					const result = await this.dialogService.confirm({
						...
						primaryButton: isString(installOptions.justification)
							...
							: nls.localize(
									{ key: 'enableButtonLabelWithAction', comment: ['&& denotes a mnemonic'] },
	...
	async uninstall(e: IExtension): Promise<void> {
		...
		if (
			!(this.productService as any).defaultChatAgent?.extensionId ||
			!areSameExtensions(extension.identifier, { id: (this.productService as any).defaultChatAgent.extensionId })
		) {
			for (const packExtension of this.getAllPackedExtensions(extension, this.local)) {
				if (
					packExtension.local &&
					!extensionsToUninstall.some(e => areSameExtensions(e.extension.identifier, packExtension.identifier))
				) {
	...
	private async installFromVSIX(vsix: URI, installOptions: InstallOptions): Promise<ILocalExtension> {
		...
		const existingExtension = this.local.find(local =>
			areSameExtensions(local.identifier, { id: getGalleryExtensionId(manifest.publisher, manifest.name) })
		);
	...
	private promptAndSetEnablement(extensions: IExtension[], enablementState: EnablementState): Promise<any> {
		const enable =
			enablementState === EnablementState.EnabledGlobally || enablementState === EnablementState.EnabledWorkspace;
		if (enable) {
	...
	private getDependentsAfterDisablement(
		...
		return installed.filter(i => {
			...
			return i.dependencies.some(dep =>
				[extension, ...extensionsToDisable].some(d => areSameExtensions(d.identifier, { id: dep }))
			);
		});
	}
