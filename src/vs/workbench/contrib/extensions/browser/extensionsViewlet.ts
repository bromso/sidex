...
export class ExtensionsViewletViewsContribution extends Disposable implements IWorkbenchContribution {
	...
	private createDefaultExtensionsViewDescriptors(): IViewDescriptor[] {
		...
		if (
			...
			installedWebExtensionsContextChangeEvent = Event.filter(this.contextKeyService.onDidChangeContext, e =>
				e.affectsSome(interestingContextKeys)
			);
		}
	...
	implements IExtensionsViewPaneContainer
{
				...
				onDrop: async (e: DragEvent) => {
					if (this.isSupportedDragElement(e)) {
						...
						const vsixs = coalesce(
							(
								await this.instantiationService.invokeFunction(accessor => extractEditorsAndFilesDropData(accessor, e))
	...
	private renderNotificaiton(): void {
		...
		if (status && (query === this.searchBox?.getValue() || !this.searchMarketplaceExtensionsContextKey.get())) {
			...
			this.notificationDisposables.value.add(
				addDisposableListener(showAction, EventType.KEY_DOWN, (e: KeyboardEvent) => {
			...
			if (status.action) {
				...
				this.notificationDisposables.value.add(
					addDisposableListener(actionButton, EventType.KEY_DOWN, (e: KeyboardEvent) => {
						const standardKeyboardEvent = new StandardKeyboardEvent(e);
						if (standardKeyboardEvent.keyCode === KeyCode.Enter || standardKeyboardEvent.keyCode === KeyCode.Space) {
							Promise.resolve(status.action!.run()).catch(error => this.notificationService.error(error));
						}
	...
	private alertSearchResult(count: number, viewId: string): void {
		...
		switch (count) {
			...
			default:
				if (view) {
					alert(
						localize('extensionsFoundInSection', '{0} extensions found in the {1} section.', count, view.name.value)
					);
				} else {
	...
	private onError(err: Error): void {
		...
		if (/ECONNREFUSED/.test(message)) {
			const error = createErrorWithActions(
				localize('suggestProxyError', "Marketplace returned 'ECONNREFUSED'. Please check the 'http.proxy' setting."),
				[
					new Action('open user settings', localize('open user settings', 'Open User Settings'), undefined, true, () =>
		...
		try {
			...
			for (const extension of this.extensionsWorkbenchService.installed) {
				if (extension.isMalicious && extension.local) {
					...
					shouldReloadWindow =
						shouldReloadWindow || extension.runtimeState?.action === ExtensionRuntimeActionType.ReloadWindow;
				}
			}
			if (maliciousExtensions.length) {
				await this.extensionsManagementService.uninstallExtensions(
					maliciousExtensions.map(e => ({ extension: e[0], options: { remove: true } }))
				);
	...
	private async updateBadge(): Promise<void> {
		...
		if (status === ExtensionGalleryManifestStatus.RequiresSignIn) {
			const badge = new NumberBadge(1, () =>
				localize('sign in enterprise marketplace', 'Sign in to access Marketplace')
			);
