	...
	protected createEditor(parent: HTMLElement): void {
		...
		const renderer: IListRenderer<IRuntimeExtension, IRuntimeExtensionTemplateData> = {
			...
			renderElement: (element: IRuntimeExtension, index: number, data: IRuntimeExtensionTemplateData): void => {
				...
				data.name.textContent = (element.marketplaceInfo?.displayName || element.description.identifier.value).substr(
					0,
					50
				);
					...
					if (accessData) {
						...
						if (accessData?.accessTimes.length > 0) {
							...
							if (accessData.current) {
								...
								data.elementDisposables.push(
									this._hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), element, title)
								);
							}
