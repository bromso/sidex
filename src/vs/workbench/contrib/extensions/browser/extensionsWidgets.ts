		...
		if (this.small) {
			...
			this.containerHover = this.disposables.add(
				this.hoverService.setupManagedHover(
					getDefaultHoverDelegate('mouse'),
					this.element,
					localize('publisher', 'Publisher ({0})', this.extension.publisherDisplayName)
				)
			);
			append(this.element, publisherDisplayName);
...
export class ExtensionKindIndicatorWidget extends ExtensionWidget {
	...
	render(): void {
		...
		if (this.extension?.private) {
			...
			if (
				!this.small ||
				(this.extensionGalleryManifest?.capabilities.extensions?.includePublicExtensions &&
					this.extensionGalleryManifest?.capabilities.extensions?.includePrivateExtensions)
			) {
			...
			if (!this.small) {
				append(
					this.element,
					$('span.private-extension-label', undefined, localize('privateExtension', 'Private Extension'))
				);
			}
			return;
		}
...
export class ExtensionRestartRequiredWidget extends ExtensionWidget {
	...
	render(): void {
		...
		if (runtimeState && /restart|reload/i.test(reason)) {
			...
			append(
				this.container,
				$('span.extension-restart-required-label', undefined, localize('restart required', 'Restart Required'))
			);
	...
	render(): void {
		...
		if (extensionStatus?.activationTimes) {
			const activationTime =
				extensionStatus.activationTimes.codeLoadingTime + extensionStatus.activationTimes.activateCallTime;
...
export class ExtensionHoverWidget extends ExtensionWidget {
	...
	private getRecommendationMessage(extension: IExtension): string | undefined {
		...
		const recommendation =
			this.extensionRecommendationsService.getAllRecommendationsWithReason()[extension.identifier.id.toLowerCase()];
...
export const extensionRatingIconColor = registerColor(
	...
	localize('extensionIconStarForeground', 'The icon color for extension ratings.'),
	false
);
export const extensionPreReleaseIconColor = registerColor(
	...
	localize('extensionPreReleaseForeground', 'The icon color for pre-release extension.'),
	false
);
export const extensionSponsorIconColor = registerColor(
	...
	localize('extensionIcon.sponsorForeground', 'The icon color for extension sponsor.'),
	false
);
export const extensionPrivateBadgeBackground = registerColor(
	'extensionIcon.privateForeground',
	{ dark: '#ffffff60', light: '#00000060', hcDark: '#ffffff60', hcLight: '#00000060' },
