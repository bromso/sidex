...
import { CompareResult } from '@sidex/base/common/arrays.js';
import { BugIndicatingError } from '@sidex/base/common/errors.js';
import { toDisposable } from '@sidex/base/common/lifecycle.js';
import { autorun, autorunWithStore, derived, IObservable } from '@sidex/base/common/observable.js';
import { localize } from '@sidex/base/nls.js';
import { IModelDeltaDecoration, MinimapPosition, OverviewRulerLane } from '@sidex/editor/common/model.js';
...
export class ResultCodeEditorView extends CodeEditorView {
	constructor(
		...
		this._register(
			autorun(reader => {
				...
				reset(
					...
					...renderLabelWithIcons(this._labelService.getUriLabel(vm.model.resultTextModel.uri, { relative: true }))
				);
			})
		);
		...
		this._register(
			createSelectionsAutorun(this, (baseRange, viewModel) => viewModel.model.translateBaseRangeToResult(baseRange))
		);

		this._register(
			instantiationService.createInstance(TitleMenu, MenuId.MergeInputResultToolbar, this.htmlElements.toolbar)
		);
	}
