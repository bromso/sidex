...
import { addDisposableListener, EventType, h, reset } from '@sidex/base/browser/dom.js';
...
export class InputCodeEditorView extends CodeEditorView {
	...
	constructor(
		...
		this._register(
			// SideX: explicit type argument; TS 6 does not infer `T` from the context-sensitive arrows below and falls back to `EditorGutter`'s `IGutterItemInfo` default.
			new EditorGutter<ModifiedBaseRangeGutterItemModel>(this.editor, this.htmlElements.gutterDiv, {
		...
		this._register(
			autorunOpts({ debugName: `input${this.inputNumber}: update labels & text model` }, reader => {
