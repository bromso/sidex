...
import { Dimension, reset } from '@sidex/base/browser/dom.js';
import { Grid, GridNodeDescriptor, IView, SerializableGrid } from '@sidex/base/browser/ui/grid/grid.js';
...
import { ICodeEditor, IViewZoneChangeAccessor } from '@sidex/editor/browser/editorBrowser.js';
...
import { IEditorOptions, ITextEditorOptions, ITextResourceEditorInput } from '@sidex/platform/editor/common/editor.js';
...
export class MergeEditor extends AbstractTextEditor<IMergeEditorViewState> {
	...
	constructor(
		...
		this.inputResultView = this._register(
			this.instantiationService.createInstance(ResultCodeEditorView, this._viewModel)
		);
	...
	// SideX: `declare` — SideX compiles with useDefineForClassFields (VS Code sets it false), so redeclaring a base property without an initializer is TS2612.
	declare readonly onDidChangeSizeConstraints: Event<void>;
		...
		transaction(tx => {
			...
			if (layout.showBase && !this.baseView.get()) {
				...
				const baseView = this.baseViewDisposables.add(
					this.instantiationService.createInstance(BaseCodeEditorView, this.viewModel)
				);
	...
	protected computeEditorViewState(resource: URI): IMergeEditorViewState | undefined {
		...
		const focusIndex = [this.input1View.editor, this.input2View.editor, this.inputResultView.editor].findIndex(editor =>
