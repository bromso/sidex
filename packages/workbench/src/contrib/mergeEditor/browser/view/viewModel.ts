...
export class MergeEditorViewModel extends Disposable {
	...
	constructor(
		...
		this.counter = 0;
		this.lastFocusedEditor = derivedObservableWithWritableCache<{ view: CodeEditorView | undefined; counter: number }>(
			this,
			(reader, lastValue) => {
				const editors = [
	...
	private goToConflict(
		getModifiedBaseRange: (editor: CodeEditorView, curLineNumber: number) => ModifiedBaseRange | undefined
	): void {
		...
		this.goToConflict(
			(e, l) =>
				this.model.modifiedBaseRanges
					.get()
					.find(r => predicate(r) && this.getRangeOfModifiedBaseRange(e, r, undefined).startLineNumber > l) ||
		...
		this.goToConflict(
			(e, l) =>
				findLast(
					...
					r => predicate(r) && this.getRangeOfModifiedBaseRange(e, r, undefined).endLineNumberExclusive < l
	...
	public toggleActiveConflict(inputNumber: 1 | 2): void {
		...
		if (!activeModifiedBaseRange) {
			this.notificationService.error(
				localize('noConflictMessage', 'There is currently no conflict focused that can be toggled.')
			);
			return;
		}
		transaction(tx => {
