...
import { compareBy, concatArrays, numberComparator } from '@sidex/base/common/arrays.js';
...
export class MappingAlignment<T extends LineRangeMapping> {
	public static compute<T extends LineRangeMapping>(
		...
		const currentDiffs = [[] as T[], [] as T[]];
		const deltaFromBaseToInput = [0, 0];

		const alignments: MappingAlignment<T>[] = [];
...
export class DetailedLineRangeMapping extends LineRangeMapping {
	...
	constructor(
		...
		this.rangeMappings = rangeMappings || [
			new RangeMapping(this.inputRange.toExclusiveRange(), this.outputRange.toExclusiveRange())
		];
	}
