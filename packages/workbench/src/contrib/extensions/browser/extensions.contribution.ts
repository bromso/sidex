...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(ExtensionEditor, ExtensionEditor.ID, localize('extension', 'Extension')),
...
class ExtensionsContributions extends Disposable implements IWorkbenchContribution {
	constructor(
		...
		extensionGalleryManifestService.getExtensionGalleryManifest().then(extensionGalleryManifest => {
			...
			this._register(
				extensionGalleryManifestService.onDidChangeExtensionGalleryManifest(extensionGalleryManifest =>
					this.updateGalleryCapabilitiesContexts(extensionGalleryManifest)
				)
			);
		});
	...
	private registerGlobalActions(): void {
		...
		this.registerExtensionAction({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					CONTEXT_HAS_GALLERY,
					ContextKeyExpr.or(CONTEXT_HAS_LOCAL_SERVER, CONTEXT_HAS_REMOTE_SERVER, CONTEXT_HAS_WEB_SERVER)
				)
			},
			run: async (accessor: ServicesAccessor) => {
		...
		this.registerExtensionAction({
			...
			run: (accessor: ServicesAccessor) =>
				accessor.get(IExtensionsWorkbenchService).updateAutoUpdateForAllExtensions(true)
		});
		...
		this.registerExtensionAction({
			...
			run: (accessor: ServicesAccessor) =>
				accessor.get(IExtensionsWorkbenchService).updateAutoUpdateForAllExtensions(false)
		});
			...
			menu: [
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						...
						ContextKeyExpr.or(
							ContextKeyExpr.has(`config.${AutoUpdateConfigurationKey}`).negate(),
							ContextKeyExpr.equals(`config.${AutoUpdateConfigurationKey}`, 'onlyEnabledExtensions')
						)
					),
					group: '1_updates',
					order: 2
			...
			run: async (accessor: ServicesAccessor) => {
				...
				const vsixPaths = await fileDialogService.showOpenDialog({
					...
					openLabel: mnemonicButtonLabel(
						localize({ key: 'installButton', comment: ['&& denotes a mnemonic'] }, '&&Install')
					)
				});
				if (vsixPaths) {
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, resources: URI[] | URI) => {
				...
				const result = await Promise.allSettled(
					vsixs.map(async vsix => await extensionsWorkbenchService.install(vsix, { installGivenVersion: true }))
				);
		...
		const featuresExtensionsWhenContext = ContextKeyExpr.and(
			CONTEXT_HAS_GALLERY,
			ContextKeyExpr.regex(CONTEXT_GALLERY_FILTER_CAPABILITIES.key, new RegExp(`_${FilterType.Featured}_`))
		);
		this.registerExtensionAction({
		...
		MenuRegistry.appendMenuItem(extensionsFilterSubMenu, {
			...
			when: ContextKeyExpr.and(
				CONTEXT_HAS_GALLERY,
				ContextKeyExpr.regex(CONTEXT_GALLERY_FILTER_CAPABILITIES.key, new RegExp(`_${FilterType.Category}_`))
			),
			group: '2_categories',
			order: 1
		});
		...
		[
			...
			this.registerExtensionAction({
				...
				precondition: ContextKeyExpr.and(
					precondition,
					ContextKeyExpr.regex(ExtensionsSearchValueContext.key, /^@contribute:/).negate(),
	...
	private registerContextMenuActions(): void {
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				const extensionWorkbenchService = accessor.get(IExtensionsWorkbenchService);
				const extension = (
					await extensionWorkbenchService.getExtensions([{ id: extensionId }], CancellationToken.None)
				)[0];
		...
		this.registerExtensionAction({
			...
			precondition: ContextKeyExpr.and(
				ContextKeyExpr.or(
					ContextKeyExpr.notEquals(`config.${AutoUpdateConfigurationKey}`, 'onlyEnabledExtensions'),
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				...
				const extensionsWorkbenchService = accessor.get(IExtensionsWorkbenchService);
				const extension = (
					await extensionsWorkbenchService.getExtensions([{ id: extensionId }], CancellationToken.None)
				)[0];
		...
		this.registerExtensionAction({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.or(
						ContextKeyExpr.and(CONTEXT_GALLERY_ALL_PUBLIC_REPOSITORY_SIGNED, ContextKeyExpr.not('extensionIsPrivate')),
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				const instantiationService = accessor.get(IInstantiationService);
				const extension =
					this.extensionsWorkbenchService.local.filter(e => areSameExtensions(e.identifier, { id: extensionId }))[0] ||
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, id: string) =>
				accessor.get(IPreferencesService).openSettings({ jsonEditor: false, query: `@ext:${id}` })
		});
		...
		this.registerExtensionAction({
			...
			run: (accessor: ServicesAccessor, id: string) =>
				accessor.get(ICommandService).executeCommand('_manageAccountPreferencesForExtension', id)
		});
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, id: string) =>
				accessor.get(IPreferencesService).openGlobalKeybindingSettings(false, { query: `@ext:${id}` })
		});
			...
			run: async (accessor: ServicesAccessor, _: string, extensionArg: IExtensionArg) => {
				...
				const extension = extensionArg.location
					? this.extensionsWorkbenchService.installed.find(e =>
							uriIdentityService.extUri.isEqual(e.local?.location, extensionArg.location)
						)
					: undefined;
				if (extension) {
		...
		this.registerExtensionAction({
			...
			run: (accessor: ServicesAccessor, id: string) =>
				accessor.get(IWorkspaceExtensionsConfigService).toggleRecommendation(id)
		});
				...
				extensionManagementService
					...
					.then(async extensions => {
						...
						for (const extension of extensions) {
							...
							if (
								productService.trustedExtensionPublishers?.includes(publisher) ||
